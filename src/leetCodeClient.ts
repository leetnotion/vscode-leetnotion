...
import { Credential, LeetCodeAdvanced, LeetCodeCLI, JudgeResult, TopVotedSolution, CategoryProblem } from "@leetnotion/leetcode-api";
import { globalState } from "./globalState";
import { extractCookie } from "./utils/toolUtils";
import { DialogType, promptForOpenOutputChannel } from "./utils/uiUtils";
import { leetCodeChannel } from "./leetCodeChannel";
import { LeetcodeProblem, ProblemRatingMap } from "./types";
import { IProblem, ProblemState, ProblemRating } from "./shared";
import { getQuestionCompanyTags, getQuestionTopicTags, getProblemRatingMap } from "./utils/dataUtils";
...
class LeetcodeClient {
    ...
    private async withSessionRetry<T>(fn: () => Promise<T>): Promise<T> {
        try {
            ...
            if (errorStr.includes("401") || errorStr.includes("403") || errorStr.includes("session") || errorStr.includes("login")) {
    ...
    public async listProblems(): Promise<IProblem[]> {
        ...
        return problems.map((p): IProblem => {
    ...
    public async getCodeTemplate(slug: string, language: string, showDescriptionInComment: boolean): Promise<string> {
        const problem = await this.leetcode.problem(slug);
        ...
        const commentLine = (language === "python" || language === "python3" || language === "ruby" || language === "bash") ? "#" : "//";
    ...
    public async getTopVotedSolution(slug: string, questionId: string, language?: string): Promise<TopVotedSolution | null> {
        return this.withSessionRetry(() => this.cli.getTopVotedSolution(slug, questionId, language));
    }

    public async submitCode(slug: string, lang: string, questionId: number, typedCode: string): Promise<JudgeResult> {
        return this.withSessionRetry(() => this.cli.submitCode({ slug, lang, questionId, typedCode }));
    }

    public async testCode(slug: string, lang: string, questionId: number, typedCode: string, dataInput: string): Promise<JudgeResult[]> {
        return this.withSessionRetry(() => this.cli.testCode({ slug, lang, questionId, typedCode, dataInput }));
    }
    ...
    private async syncFavoriteToApi(questionId: string, addToFavorite: boolean): Promise<void> {
        try {
            leetCodeChannel.appendLine(`Syncing favorite for problem ${questionId}: ${addToFavorite ? "star" : "unstar"}`);
    ...
    private getFavoriteState(questionId: string, apiValue: boolean): boolean {
        const overrides: Record<string, boolean> | undefined = globalState.get("leetcode-favorite-overrides");
    ...
    public deleteCache(): void {
        globalState.deleteLeetCodeCache();
    }
}
