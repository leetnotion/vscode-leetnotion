...
class MarkdownEngine implements vscode.Disposable {
	...
	private getBuiltinStyles(webview: vscode.Webview): string {
		...
		try {
			const stylePaths: string[] = require(path.join(extRoot, 'package.json'))['contributes'][
...
class MarkdownConfiguration {
	...
	public constructor() {
		const markdownConfig: vscode.WorkspaceConfiguration = vscode.workspace.getConfiguration(
			'markdown',
			null,
		);
