...
class LeetCodeSubmissionProvider extends LeetCodeWebview {
	...
	protected getWebviewContent(): string {
		...
		const showAnalyzeButton =
			this.isAccepted && !this.isTest && this.submittedCode && isAIAvailable();
			...
			if (this.problemSlug) {
				try {
					...
					leetCodeChannel.appendLine(
						'[AI] Failed to fetch problem description, proceeding without it.',
					);
				}
			}
	...
	private async handleDebug(): Promise<void> {
		try {
			...
			const expectedStr = judge?.expected_answer
				? Array.isArray(judge.expected_answer)
