		...
		try {
			if (!hasNotionIntegrationEnabled()) {
				leetCodeChannel.appendLine(
					`Failed to update notion template. Notion integration not enabled. Enable notion integration and complete the setup.`,
				);
				promptForOpenOutputChannel(
					`Failed to update notion template. Notion integration not enabled.`,
					DialogType.error,
				);
				return;
			}
			...
			promptForOpenOutputChannel(
				`Updated leetnotion template successfully 🥳.`,
			...
			await window.withProgress(
				...
				async (progress) => {
					...
					problems = await leetcodeClient.getLeetcodeProblems((problems) => {
						...
						leetCodeChannel.appendLine(
							`Fetched ${problems.length}/${noOfProblems} problems from leetcode.`,
						);
					});
				},
			);
