...
import {
	LeetcodeProblem,
	LeetcodeSubmission,
	LeetnotionSubmission,
	Mapping,
	MultiSelectDatabasePropertyConfigResponse,
	ProblemPageResponse,
	QueryProblemPageProperties,
	SelectTags,
...
class LeetnotionClient {
	...
	public async updateTemplateInformation(callbackFn: () => void = () => {}) {
		...
		let pages: ProblemPageResponse[] = (await this.notion?.getAllPages(
			questionsDatabaseId as string,
			callbackFn,
		)) as ProblemPageResponse[];

		const questionNumberPageIdMapping: Mapping = {};
		pages
			.filter((page) => page.properties['Question Number'].number !== null)
			.forEach((page) => {
	...
	public async updateStatusOfQuestion(
		questionNumber: string,
		check: boolean = true,
	): Promise<PageObjectResponse<QueryProblemPageProperties>> {
		try {
	...
	public async createSubmissionPage(
		questionNumber: string,
	...
	public async setUserQuestionTags(): Promise<void> {
		...
		try {
			...
			const tags = databaseProperties.properties[
				'Tags'
	...
	public async addProblems(
		problems: LeetcodeProblem[],
		callbackFn: (response: ProblemPageResponse) => void = () => {},
		...
		try {
			...
			leetCodeChannel.appendLine(
				`Adding ${problems.length} problems to Notion database with ID: ${databaseId}`,
			);
			leetCodeChannel.appendLine(
				`Adding ${problems.length} problems to Notion database with ID: ${globalState.getSubmissionsDatabaseId()}`,
			);
			if (!databaseId) {
				throw new Error(`questions-database-id-not-found`);
			}
	...
	public async getSubmissionPages(
		...
		try {
			...
			if (questionsMissing) {
				promptForOpenOutputChannel(
					`Few questions are missing in the template. Submissions of other questions are added. Update the template using 'Update leetnotion template' command and run 'Add existing submissions to template' command again to add remaining submissions to template`,
					DialogType.warning,
				);
			} else {
				promptForOpenOutputChannel(
					`Successfully added submissions to template`,
